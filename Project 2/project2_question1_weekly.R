...
#        .[, YEAR_ONE := WEEK_NO < 52] %>% 
        ...
        summarise(SUM_SALES=sum(SALES_VALUE)
                  , NUM_TRIPS = length(unique(BASKET_ID))) %>% 
        data.table()
...
avg_basket <- transaction_data %>%
        group_by(household_key, WEEK_NO, BASKET_ID) %>%
        summarise(BASKET_SIZE=sum(SALES_VALUE)) %>%
        data.table() %>%
        group_by(household_key, WEEK_NO) %>%
        summarise(AVG_BASKET_SIZE=mean(BASKET_SIZE)) %>%
        data.table() %>% 
        select(household_key, WEEK_NO, AVG_BASKET_SIZE)

summary_table2 <- summary_table %>%
        inner_join(avg_basket) %>% 
#        inner_join(earliest_week) %>% 
        data.table(); 
rm(summary_table); rm(avg_basket)


# join demographic variables
summary_table3 <- summary_table2 %>% inner_join(hh_demographic) %>% data.table() %>% .[WEEK_NO>22]; 
rm(summary_table2)

summary_table3 %>% .[, income_level:=""] %>% 
...
# names(summary_table3)
# table(summary_table3$INCOME_DESC)
...
# # Writing to csv file
# write.csv(x = summary_table3
#           , file = '/Users/kjgardner/Dropbox/MSBA\ Program/Fall\ 2016/BZAN\ 535/Project\ 2/weekly_sum_sales_by_hh.csv'
#           , row.names = FALSE)
